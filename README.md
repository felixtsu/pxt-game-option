...
* **弹出升级选择(2个)** — 传入 2 个 `upgradeOption` 变量
* **弹出升级选择(3个)** — 传入 3 个 `upgradeOption` 变量

没有 `choose(标题, 选项数组)` 这种写法。旧版若还有 `弹出升级选择 ... 选项 options`，请删掉改用下面两条之一。
...
```blocks
let optA = gameoption.create(图标1, "攻速 +20%")
let optB = gameoption.create(图标2, "伤害 +15%")
...
```blocks
let optA = gameoption.create(img`...`, "攻速 +20%")
let optB = gameoption.create(img`...`, "伤害 +15%")
let choice = gameoption.choose2("选择升级", optA, optB)
```
